"use client";

import Image from "next/image";
import Link from "next/link";

const menuItems = [
  { name: "My Groups", href: "/Dashboard/group/create", icon: "/images/groups.png" },
  { name: "Join Requests", href: "/requests", icon: "/images/requests.png" },
  { name: "Assignments", href: "/assignments", icon: "/images/assignments.png" },
  { name: "Students", href: "/students", icon: "/images/studentss.png" },
  { name: "Analytics", href: "/analytics", icon: "/images/analytics.png" },
];

const Items = [
  { name: "Profile", href: "/profile", icon: "/images/profile.png" },
  { name: "Settings", href: "/settings", icon: "/images/setting.png" },
];

export default function DesktopSidebar() {
  return (
    <aside className="hidden h-full w-72 shrink-0 flex-col overflow-hidden bg-[#E6E3D0] p-4 lg:flex">
      
      <div className="mb-5 flex items-center gap-3">
        
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary">
          <Image
            src="/images/teacherportal.png"
            alt="Teacher Logo"
            width={22}
            height={22}
          />
        </div>

        <div className="leading-tight">
          <h2 className="font-sans text-sm font-bold leading-5 tracking-[0.14px] text-primary">
            Teacher Portal
          </h2>

          <p className="text-[10px] leading-[15px] tracking-[0.5px] uppercase font-normal text-[#48473A]">
            Academic Excellence
          </p>
        </div>
      </div>

      <Link
        href="/Dashboard"
        className="mb-4 flex w-full items-center gap-3 rounded-xl bg-primary px-4 py-2.5 text-sm text-white transition hover:opacity-90"
      >
        <Image
          src="/images/dashboard.png"
          alt="Dashboard Icon"
          width={18}
          height={18}
        />
        <span className="font-medium">Dashboard</span>
      </Link>

      <nav className="flex flex-col gap-1">
        {menuItems.map((item) => (
          <Link
            key={item.name}
            href={item.href}
            className="flex items-center gap-3 rounded-xl px-4 py-2.5 text-sm transition hover:bg-white/40 text-[#48473A]"
          >
            <Image src={item.icon} alt={item.name} width={20} height={20} />
            <span>{item.name}</span>
          </Link>
        ))}
      </nav>

      <div className="my-4 border-t border-text/10" />

      <div className="flex flex-col gap-1">
        {Items.map((item) => (
          <Link
            key={item.name}
            href={item.href}
            className="flex items-center gap-3 rounded-xl px-4 py-2.5 text-sm transition hover:bg-white/40 text-[#48473A]"
          >
            <Image src={item.icon} alt={item.name} width={20} height={20} />
            <span>{item.name}</span>
          </Link>
        ))}
      </div>

      <Link
        href="/Dashboard/group/create"
        className="mt-auto flex items-center justify-center gap-3 rounded-xl bg-primary px-4 py-2.5 text-sm text-white shadow-md transition hover:opacity-90"
      >
        <Image
          src="/images/plus.png"
          alt="Create Icon"
          width={18}
          height={18}
        />
        <span className="font-medium">Create New Group</span>
      </Link>
    </aside>
  );
}