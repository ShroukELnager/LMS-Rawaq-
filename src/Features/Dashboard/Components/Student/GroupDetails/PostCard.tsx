...
return (
  <div className="rounded-2xl bg-white p-5 shadow-sm">
    ...
    <div className="mt-5 flex items-center gap-8 border-b border-[#EAECF0] pb-4">
      <button
        onClick={handleLike}
        className={`flex items-center gap-2 text-sm ${post.is_liked ? 'text-primary' : 'text-[#667085]'}`}
      >
      ...
      <button
        ...
        className="flex items-center gap-2 text-sm text-[#667085] cursor-pointer"
      >
        <Comment size={18} />
      ...
      <>
        <div className="mt-5 space-y-4">
          ...
          {!commentsLoading && comments.length === 0 && (
            <div className="flex flex-col items-center justify-center rounded-2xl bg-[#F9FAFB] py-8 text-center">
          ...
          {comments.map((comment) => (
            <div key={comment.id} className="flex items-start gap-3">
              ...
              <div className="flex flex-col">
                <div className="max-w-[320px] rounded-2xl rounded-tl-md bg-[#E7EEFF] px-4 py-2.5">
                  <h4 className="text-sm font-semibold text-[#050505]">
                    {comment.author?.first_name} {comment.author?.last_name}
                  </h4>
                  <p className="mt-1 break-words text-sm leading-5 text-[#050505]">
                    {comment.content}
                  </p>
                </div>

                <div className="mt-1 px-3 text-xs text-[#65676B]">
        ...
        <div className="mt-5 flex items-center gap-3">
          ...
          <div className="relative flex-1">
            <input
              ...
              className="h-11 w-full rounded-xl border border-[#D0D5DD] bg-[#EEF4FF] pl-4 pr-12 text-sm outline-none focus:border-primary focus:bg-white"
            />

            <button
              onClick={handlePublish}
              disabled={isPending || !content.trim()}
              className="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer text-primary disabled:opacity-40"
            >
              {isPending ? (
