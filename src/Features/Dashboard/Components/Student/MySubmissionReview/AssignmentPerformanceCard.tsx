...
export default function AssignmentPerformanceCard({
  ...
  return (
    <>
    <div className="flex w-full gap-6 hidden md:flex">
...
<div className="w-full md:hidden">
  <div className="w-full rounded-xl border border-[#E5E7EB] bg-white p-4 shadow-sm">
    <div className="grid grid-cols-2 gap-x-6 gap-y-5">
      <InfoItem
        label="Status"
        value={
          isReviewed ? (
            <span className="inline-flex items-center gap-1 rounded-full bg-[#E7F6F3] px-2 py-1 font-inter text-[10px] font-semibold leading-3 text-primary">
              <CheckCircle2 size={12} strokeWidth={2.5} />
              Reviewed
            </span>
          ) : (
            <span className="inline-flex items-center gap-1 rounded-full bg-[#FFF7E6] px-2 py-1 font-inter text-[10px] font-semibold leading-3 text-[#B45309]">
              Waiting
            </span>
          )
        }
      ...
      <InfoItem
        label="Total Points"
        value={`${totalGrade} Points`}
        valueClass="font-inter text-xs font-bold text-[#111C2C]"
      />

      <InfoItem
        label="Grade Awarded"
        value={isReviewed ? `${awardedGrade} Points` : 'Not graded'}
        valueClass="font-inter text-xs font-bold text-primary"
      />

      <InfoItem
        label="Deadline"
        value={assignment?.deadline ? formatDate(assignment.deadline) : '-'}
        valueClass="font-inter text-xs font-normal leading-4 text-[#111C2C]"
      />
    ...
    <div className="mt-5 border-t border-[#E5E7EB] pt-4">
      <p className="font-inter text-[10px] font-medium text-[#6F797A]">
        General Description
      </p>

      <p className="mt-2 font-inter text-xs font-normal leading-5 text-[#3E494A]">
        {assignment?.description || '-'}
      </p>
    </div>
  </div>
